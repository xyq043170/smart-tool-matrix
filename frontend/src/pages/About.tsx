...
import { ABOUT, ABOUT_EN } from '@/config/constants'
...
export default function AboutPage() {
  ...
  const aboutContent = i18n.language === 'zh' ? ABOUT : ABOUT_EN
    ...
    <div className="space-y-4 md:space-y-6 animate-in fade-in duration-500">
      <Card className="backdrop-blur-lg bg-card/50 shadow-xl border-primary/10 hover:border-primary/20 transition-all relative">
        ...
        >
          ...
          <span className="text-xs md:text-sm">{t('nav.back')}</span>
        </Button>
        <CardContent className="pt-12 md:pt-14 px-4 md:px-6 pb-4 md:pb-6">
          <div
            ...
            dangerouslySetInnerHTML={{ __html: md.render(aboutContent) }}
          />
        </CardContent>
      </Card>
    </div>
